...
namespace dconstruct::testing {
    ...
    TEST(DISASSEMBLER, EmptyFile) {
        const std::string filepath = "test/fixtures/dc/empty.bin";
        auto not_found = BinaryFile::from_path(filepath);
        ASSERT_FALSE(not_found.has_value());
    ...
    TEST(DISASSEMBLER, NonDCFileMagic) {
        const std::string filepath = "test/fixtures/dc/random.bin";
        auto not_found = BinaryFile::from_path(filepath);
        ASSERT_FALSE(not_found.has_value());
    ...
    TEST(DISASSEMBLER, NonDCFileVersion) {
        const std::string filepath = "test/fixtures/dc/random2.bin";
        auto not_found = BinaryFile::from_path(filepath);
        ASSERT_FALSE(not_found.has_value());
    ...
    TEST(DISASSEMBLER, UC4Funcs) {
        const std::string filepath = "test/fixtures/uc4/ss-isl-cave-get-piton.bin";
        const std::string graph = "test/fixtures/uc4/graph.svg";
