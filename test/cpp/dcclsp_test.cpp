...
namespace dconstruct::testing {
    ...
    static u64 idx(const std::string& text, lsp::uint line, lsp::uint character) {
        return dcclsp::position_to_string_index(text, lsp::Position{ .line = line, .character = character });
    }

    TEST(DCCLSP, EmptyDocument) {
        EXPECT_EQ(idx("", 0, 0), 0u);
    }

    TEST(DCCLSP, StartOfDocument) {
        EXPECT_EQ(idx("hello world", 0, 0), 0u);
    }

    TEST(DCCLSP, MiddleOfFirstLine) {
        EXPECT_EQ(idx("hello world", 0, 6), 6u);
    }

    TEST(DCCLSP, EndOfFirstLine) {
        EXPECT_EQ(idx("hello world", 0, 11), 11u);
    }

    TEST(DCCLSP, SecondLineStartLF) {
        EXPECT_EQ(idx("abc\ndef\nghi", 1, 0), 4u);
    }

    TEST(DCCLSP, SecondLineMidLF) {
        EXPECT_EQ(idx("abc\ndef\nghi", 1, 3), 7u);
    }

    TEST(DCCLSP, ThirdLineStartLF) {
        EXPECT_EQ(idx("abc\ndef\nghi", 2, 0), 8u);
    }

    TEST(DCCLSP, ThirdLineMidLF) {
        EXPECT_EQ(idx("abc\ndef\nghi", 2, 3), 11u);
    }

    TEST(DCCLSP, MidColumnsSecondLine) {
        EXPECT_EQ(idx("hello\nworld\nfoo", 1, 2), 8u);
        EXPECT_EQ(idx("hello\nworld\nfoo", 1, 4), 10u);
    }

    TEST(DCCLSP, CRLFSecondLine) {
        EXPECT_EQ(idx("abc\r\ndef", 1, 0), 5u);
        EXPECT_EQ(idx("abc\r\ndef", 1, 3), 8u);
    }

    TEST(DCCLSP, CROnlySecondLine) {
        EXPECT_EQ(idx("abc\rdef", 1, 0), 4u);
        EXPECT_EQ(idx("abc\rdef", 1, 3), 7u);
    }

    TEST(DCCLSP, LastLineNoTrailingNewline) {
        EXPECT_EQ(idx("a\nbb", 1, 0), 2u);
        EXPECT_EQ(idx("a\nbb", 1, 2), 4u);
    }

    TEST(DCCLSP, LeadingEmptyLine) {
        EXPECT_EQ(idx("\nabc", 0, 0), 0u);
        EXPECT_EQ(idx("\nabc", 1, 3), 4u);
    }

    TEST(DCCLSP, EmptyLineInMiddleAddressable) {
        EXPECT_EQ(idx("a\n\nb", 1, 0), 2u);
    }

    TEST(DCCLSP, TrailingEmptyLineAddressable) {
        EXPECT_EQ(idx("abc\n", 1, 0), 4u);
    }

    TEST(DCCLSP, LineBeyondDocumentClampsToSize) {
        const std::string text = "abc\ndef";
        EXPECT_EQ(idx(text, 5, 0), text.size());
    }

    TEST(DCCLSP, RangeRoundTripWholeDocument) {
        const std::string text = "abc\ndef\nghi";
    ...
    TEST(DCCLSP, RangeRoundTripMidToMid) {
        const std::string text = "abc\ndef\nghi";
    ...
    TEST(DCCLSP, ExtractsNeedleAtDocumentEnd) {
        dcclsp::server server{ make_sidbase({ { 1, "player-health" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, " #player-health", 0, 15);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(has_label(result, "player-health"));
    }

    TEST(DCCLSP, NeedleExcludesTextBeforeHash) {
        dcclsp::server server{ make_sidbase({ { 1, "abc" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, " xyz#abc", 0, 8);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(has_label(result, "abc"));
    }

    TEST(DCCLSP, UsesNearestHashWhenSeveralPresent) {
        dcclsp::server server{ make_sidbase({ { 1, "bar" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, " #foo #bar", 0, 10);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(has_label(result, "bar"));
    }

    TEST(DCCLSP, NeedleExtractedOnSecondLine) {
        dcclsp::server server{ make_sidbase({ { 1, "jump" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, "line one\n #jump", 1, 6);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(has_label(result, "jump"));
    }

    TEST(DCCLSP, CursorInMiddleDoesNotConsumeTrailingChar) {
        dcclsp::server server{ make_sidbase({ { 1, "foo" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, " #foo bar", 0, 5);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(item_count(result) == 0);
    }

    TEST(DCCLSP, HashAtDocumentStart) {
        dcclsp::server server{ make_sidbase({ { 1, "foo" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, "#foo", 0, 4);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_TRUE(has_label(result, "foo"));
    }

    TEST(DCCLSP, EmptyNeedleYieldsNoItems) {
        dcclsp::server server{ make_sidbase({ { 1, "foo" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        auto result = dcclsp::server_test_access::complete(server, uri, " #", 0, 2);

        EXPECT_FALSE(dcclsp::server_test_access::errored(server));
        EXPECT_EQ(item_count(result), 0u);
    }

    TEST(DCCLSP, NoHashBeforeCursorErrors) {
        dcclsp::server server{ make_sidbase({ { 1, "foo" } }) };
        const lsp::Uri uri = lsp::Uri::parse("file:///a.dcpl");
        dcclsp::server_test_access::complete(server, uri, "no hash here", 0, 12);

        EXPECT_TRUE(dcclsp::server_test_access::errored(server));
    }
