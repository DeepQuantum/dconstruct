...
namespace dconstruct::ast {
    ...
    [[nodiscard]] resstr<compilation::program_binary_element> state_script::emit_dc(compilation::global_state& global) const noexcept {
        ...
        const u64 total_declarations_size = std::accumulate(
            m_declarations.begin(),
            m_declarations.end(),
            0ull,
            [](u64 sum, const variable_declaration& decl) {
        ...
        const auto declaration_type_sid = [](const primitive_type& ptype) noexcept -> sid64 {
            switch (ptype.m_type) {
                ...
                case primitive_kind::SID:
                    return SID("symbol");
                default:
                    assert(false && "unsupported declaration type");
                    return SID("int32");
            }
        };
        ...
        element.push_bytes(
            ...
            if (!m_options.empty()) {
                ...
                element.push_bytes(ss_options, 0b0111'1001,
            0b00
        );
                ...
                element.push_bytes(
                    ...
                    for (u32 i = 0; i < m_states.size(); ++i) {
                        ...
                        for (const auto& block : state.m_blocks) {
                            ...
                            element.push_bytes(on_block, 0b0110'0100,
                    0b10
                );
            }
        }
