...
namespace dconstruct::ast {
    ...
    void program::insert_into_bytestream(
    ...
    void program::insert_into_reloctable(
        u8* out,
        u64& byte_offset,
        u64& bit_offset,
        const u8 bits,
    ...
    [[nodiscard]] std::expected<std::pair<std::unique_ptr<std::byte[]>, u64>, std::string> program::make_binary(
        ...
        const u64 entries_data_size = std::accumulate(
            program_elements.begin(),
            program_elements.end(),
            u64{0},
            [](u64 acc, const compilation::program_binary_element& program_element) {
                return acc + program_element.m_rawData.size();
            }
        );

        const u64 stringtable_size = std::accumulate(
            global.m_strings.begin(),
            global.m_strings.end(),
            u64{0},
