...
void cast_expr::pseudo(std::ostream& os) const {
    os << '(' << type_to_declaration_string(m_type) << ')' << *m_expr; 
}

void cast_expr::ast(std::ostream& os) const {
    os << "cast[type=" << type_to_declaration_string(m_type) << ",expr=" << *m_expr << ']';
}

[[nodiscard]] expr_uptr cast_expr::simplify() const {
    return std::make_unique<cast_expr>(m_type, m_expr->simplify());
}


[[nodiscard]] expr_uptr cast_expr::clone() const {
    return std::make_unique<cast_expr>(m_type, m_expr->clone());
}
