...
namespace dconstruct::ast {
    ...
    [[nodiscard]] resstr<u16> sid_identifier::emit_to_symbol_table(
    ...
    [[nodiscard]] resstr<reg_idx> sid_identifier::emit_dc_callee(
        compilation::function_context& fn,
        ...
        resstr<reg_idx> true_destination = fn.fix_destination(destination);
        if (!true_destination) {
    ...
    [[nodiscard]] resstr<reg_idx> sid_identifier::emit_dc(
        compilation::function_context& fn,
        compilation::global_state& global,
        const std::optional<reg_idx> destination
    ) const noexcept {
