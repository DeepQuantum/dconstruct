...
[[nodiscard]] full_type grouping::compute_type_unchecked(const compilation::scope& env) const noexcept {
    return m_expr->compute_type_unchecked(env);
}

[[nodiscard]] semantic_check_res grouping::compute_type_checked(compilation::scope& env) const noexcept {
