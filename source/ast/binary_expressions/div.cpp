...
namespace dconstruct::ast {
    ...
    [[nodiscard]] semantic_check_res div_expr::compute_type_checked(compilation::scope& env) const noexcept {
        ...
        const std::expected<full_type, std::string> valid_div = std::visit(
            [](auto&& lhs_type, auto&& rhs_type) -> std::expected<full_type, std::string> {
                ...
                if constexpr (!is_primitive<lhs_t>) {
                    return std::unexpected{"expected primitive type for division but got " + type_to_declaration_string(lhs_type)};
                } else if constexpr (!is_primitive<rhs_t>) {
                    return std::unexpected{"expected primitive type for division but got " + type_to_declaration_string(rhs_type)};
                } else if (is_integral(lhs_type.m_type)) {
                    if (is_integral(rhs_type.m_type)) {
                        return make_type_from_prim(primitive_kind::U64);
                    }
                    return std::unexpected{"expected integral type for division rhs but got " + type_to_declaration_string(rhs_type)};
                    ...
                    return std::unexpected{"expected floating point type for division rhs but got " + type_to_declaration_string(rhs_type)};
                } else {
                    return std::unexpected{"expected numeric type for division but got " + type_to_declaration_string(lhs_type)};
                }
            },
            *lhs_type,
            *rhs_type
    ...
    [[nodiscard]] emission_res div_expr::emit_dc(
        ...
        const emission_res div_destination = fn.fix_destination(destination);
        if (!div_destination) {
