...
[[nodiscard]] semantic_check_res compare_expr::compute_type_checked(compilation::scope& env) const noexcept {
    ...
    const std::optional<std::string> invalid_compare = std::visit([](auto&& lhs_type, auto&& rhs_type) -> std::optional<std::string> {
        ...
        if constexpr (!is_primitive<lhs_t>) {
            return "expected primitive type for compare lhs but got " + type_to_declaration_string(lhs_type);
        } else if constexpr (!is_primitive<rhs_t>) {
            return "expected primitive type for compare rhs but got " + type_to_declaration_string(rhs_type);
            ...
            return "expected compatible types for comparison but got " + type_to_declaration_string(lhs_type) + " and " + type_to_declaration_string(rhs_type);
        }
    }, *lhs_type, *rhs_type);
