...
namespace dconstruct::ast {
        ...
        return std::visit(
            [](auto&& arg) -> primitive_kind {
                ...
                if constexpr (std::is_same_v<T, u8>)
                    return primitive_kind::U8;
                else if constexpr (std::is_same_v<T, u16>)
                    return primitive_kind::U16;
                else if constexpr (std::is_same_v<T, u32>)
                    return primitive_kind::U32;
                else if constexpr (std::is_same_v<T, u64>)
                    return primitive_kind::U64;
                else if constexpr (std::is_same_v<T, i8>)
                    return primitive_kind::I8;
                else if constexpr (std::is_same_v<T, i16>)
                    return primitive_kind::I16;
                else if constexpr (std::is_same_v<T, i32>)
                    return primitive_kind::I32;
                else if constexpr (std::is_same_v<T, i64>)
                    return primitive_kind::I64;
                else if constexpr (std::is_same_v<T, f32>)
                    return primitive_kind::F32;
                else if constexpr (std::is_same_v<T, f64>)
                    return primitive_kind::F64;
                else if constexpr (std::is_same_v<T, char>)
                    return primitive_kind::CHAR;
                else if constexpr (std::is_same_v<T, bool>)
                    return primitive_kind::BOOL;
                else if constexpr (std::is_same_v<T, std::string>)
                    return primitive_kind::STRING;
                else if constexpr (std::is_same_v<T, std::nullptr_t>)
                    return primitive_kind::NULLPTR;
                else
                    return primitive_kind::NOTHING;
            },
            prim
        );
    }
    ...
    [[nodiscard]] std::optional<primitive_number> get_number(const primitive_value& prim) noexcept {
        return std::visit(
            [](auto&& arg) -> std::optional<primitive_number> {
                using T = std::decay_t<decltype(arg)>;
        ...
        return std::visit(
            [](auto&& arg) -> std::string {
                ...
                if constexpr (std::is_same_v<T, struct_type> || std::is_same_v<T, enum_type>) {
                    return arg.m_typeHash ? '#' + arg.m_name : arg.m_name;
            ...
            [](auto&& arg) -> std::expected<Opcode, std::string> {
                ...
                if constexpr (std::is_same_v<T, primitive_type>) {
                    ...
                    switch (kind) {
                        ...
                        case primitive_kind::U8:
                            return Opcode::LoadU8;
                        case primitive_kind::I16:
                            return Opcode::LoadI16;
                        case primitive_kind::U16:
                            return Opcode::LoadU16;
                        case primitive_kind::I32:
                            return Opcode::LoadI32;
                        case primitive_kind::U32:
                            return Opcode::LoadU32;
                        case primitive_kind::I64:
                            return Opcode::LoadI64;
                        case primitive_kind::U64:
                            return Opcode::LoadU64;
                        case primitive_kind::F32:
                            return Opcode::LoadFloat;
                        default:
                            return std::unexpected{"no load opcode for primitive type " + type_to_declaration_string(arg)};
                    }
                } else if constexpr (std::is_same_v<T, ptr_type>) {
        ...
        return std::visit(
            [](auto&& arg) -> std::expected<Opcode, std::string> {
                ...
                if constexpr (std::is_same_v<T, primitive_type>) {
                    ...
                    switch (kind) {
                        case primitive_kind::I8:
                            return Opcode::StoreI8;
                        case primitive_kind::U8:
                            return Opcode::StoreU8;
                        case primitive_kind::I16:
                            return Opcode::StoreI16;
                        case primitive_kind::U16:
                            return Opcode::StoreU16;
                        case primitive_kind::I32:
                            return Opcode::StoreI32;
                        case primitive_kind::U32:
                            return Opcode::StoreU32;
                        case primitive_kind::I64:
                            return Opcode::StoreI64;
                        case primitive_kind::U64:
                            return Opcode::StoreU64;
                        case primitive_kind::F32:
                            return Opcode::StoreFloat;
                        default:
                            return std::unexpected{"no store opcode for primitive type " + type_to_declaration_string(arg)};
                    }
                } else if constexpr (std::is_same_v<T, ptr_type>) {
    ...
    [[nodiscard]] std::optional<primitive_kind> get_dominating_prim(
