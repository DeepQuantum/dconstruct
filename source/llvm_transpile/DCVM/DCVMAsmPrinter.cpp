...
void DCVMAsmPrinter::emitInstruction(const MachineInstr *MI) {
    ...
    m_codeEmitter->encodeInstruction(TmpInst, Encoded, Fixups,
                                     MF->getSubtarget());
...
void DCVMAsmPrinter::emitFunctionBodyEnd() {
    ...
    if (const MDNode *Md = F.getMetadata("dcvm.sid_distance"))
        Sid = cast<ConstantInt>(
                  cast<ConstantAsMetadata>(Md->getOperand(1))->getValue())
                  ->getZExtValue();

    appendU64LE(g_byteCode, Sid);
    appendU64LE(g_byteCode,
                static_cast<uint64_t>(InstrBytes) |
                    (static_cast<uint64_t>(SymbolBytes) << 32));
...
extern "C" const char *DCVMBytecodeData() { return g_byteCode.data(); }

extern "C" size_t DCVMBytecodeSize() { return g_byteCode.size(); }

extern "C" void DCVMBytecodeReset() { g_byteCode.clear(); }
