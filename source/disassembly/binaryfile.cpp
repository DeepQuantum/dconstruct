...
BinaryFile::BinaryFile(const std::string &path) {
    ...
    this->m_bytes = new u8[m_size];

    scriptstream.read((char*)m_bytes, m_size);
}
...
i32 BinaryFile::disassemble_file(const SIDBase &sidbase) {
    ...
    this->m_dcheader = reinterpret_cast<DC_Header*>(this->m_bytes);

    if (this->m_dcheader->m_magic != magic) {
        throw std::runtime_error("Magic number doesn't equal 0x44433030: " + std::to_string(*(uint32_t*)this->m_bytes));
    }

    if (this->m_dcheader->m_versionNumber != version) {
        throw std::runtime_error("Version number doesn't equal 0x00000001: " + std::to_string(*(uint32_t*)(this->m_bytes + 8)));
    }
...
void BinaryFile::read_reloc_table() {
    u8 *reloc_data = this->m_bytes + this->m_dcheader->m_textSize;
    ...
    for (u64 i = 0; i < table_size * 8; ++i) {
        if (bitmap[i / 8] & (1 << (i % 8))) {
            u64* entry = reinterpret_cast<u64*>(this->m_bytes + i * 8);
            u64 offset = *entry;
            *entry = reinterpret_cast<u64>(this->m_bytes + offset);
