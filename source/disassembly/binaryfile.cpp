...
BinaryFile::BinaryFile(const std::string &path) {
    ...
    u8 *temp_buffer = new(std::align_val_t(64)) u8[m_size];
...
void BinaryFile::read_reloc_table() {
    ...
    u8 *temp = new(std::align_val_t(64)) u8[table_size * 8];
    this->m_pointedAtTable = std::unique_ptr<u8[]>(temp);
    
    const u8 *bitmap = reloc_data + 4;

    printf("size: %d\n", this->m_dcheader->m_textSize);
...
#define DERANGED
    ...
    for (u64 i = 0; i < table_size; ++i) {
        ...
        __m512i _data_segment = _mm512_load_epi64((void *)data_segment_ptr);
