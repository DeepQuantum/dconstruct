...
void EditDisassembler::apply_edit(const u64 struct_location, const u32 member_index, const BinaryFileEdit &value) noexcept { 
    const p64 struct_member_start = reinterpret_cast<p64>(m_currentFile->m_bytes.get()) + struct_location;
    ...
    if (edit_member_size == 8 && (value.m_editType != EditType::SID_STR || value.m_editType != EditType::SID_HASH || value.m_editType != EditType::PTR)) {
        std::cout << "warning: member " << member_index << " of struct at location 0x" << std::hex << struct_location 
    ...
    const p64 edit_location = struct_member_start + member_location;
    std::cout << "applying change at location 0x" << std::hex << struct_location << '[' << member_index << "]: " << std::dec;
    switch(value.m_editType) {
        case EditType::F4: {
            std::cout << *reinterpret_cast<f32*>(edit_location) << '=' << value.f32 << '\n';
            *reinterpret_cast<f32*>(edit_location) = value.f32;
            break;
        }
        case EditType::INT4: {
            std::cout << *reinterpret_cast<i32*>(edit_location) << '=' << value.i32 << '\n';
            *reinterpret_cast<i32*>(edit_location) = value.i32;
            break;
        }
        case EditType::PTR: {
            std::cout << *reinterpret_cast<u64*>(edit_location) << '=' << value.u64 << '\n';
            *reinterpret_cast<u64*>(edit_location) = value.u64;
            break;
        }
        case EditType::SID_HASH: {
            const char *old_sid = lookup(*reinterpret_cast<u64*>(edit_location));
