...
void SIDBase::load(const char *path) noexcept {
    ...
    this->m_sidbytes = new u8[fsize];

    sidfile.read(reinterpret_cast<char*>(&this->m_num_entries), 8);
    sidfile.seekg(0);
    sidfile.read(reinterpret_cast<char*>(this->m_sidbytes), fsize);

    this->m_num_entries = m_num_entries;
    this->m_entries = reinterpret_cast<SIDBaseEntry*>(this->m_sidbytes + 8);
}
